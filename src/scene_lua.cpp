...
int gr_nh_plane_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new NonhierPlane());
...
int gr_material_cmd(lua_State* L)
{
  ...
  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     ...
                                     shininess, 0.0f, 1.0f, false, true);
...
extern "C"
int gr_mirror_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess, glossiness, 1.0f, true, true);
...
// Create a non-opaque Material
extern "C"
int gr_clear_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double shininess = luaL_checknumber(L, 3);
  float glossiness = luaL_checknumber(L, 4);
  float eta = luaL_checknumber(L, 5);
...
extern "C"
int gr_stripe_material_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double scale_factor = luaL_checknumber(L, 4);
  bool horizontal = lua_toboolean(L, 5);
  double shininess = luaL_checknumber(L, 6);

  StripedTexture *texture = new StripedTexture(glm::vec3(col1[0], col1[1], col1[2]),
                                                   glm::vec3(col2[0], col2[1], col2[2]),
...
int gr_image_material_cmd(lua_State* L)
{
  ...
  bool flip_v = lua_toboolean(L, 5);
  bool ignore_lighting = lua_toboolean(L, 6);

  ImageTexture *texture = new ImageTexture(filename, factor[0], factor[1], flip_v);
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"stripe_material", gr_stripe_material_cmd},
